...
public class Colony extends Settlement implements Nameable, TradeLocation {
	...
	/**
	 * The number of liberty points. Liberty points are an abstract game concept.
	 * They are generated by but are not identical to bells, and subject to further
	 * modification.
	 */
	...
	/**
	 * Create a new <code>Colony</code> with the given identifier. The object should
	 ...
	 *
	 * Lower level routine, do not use directly in-game (use buildBuilding). Used
	 * for serialization and public for the test suite.
	 *
	 ...
	 * @return A <code>Building</code> which produces the given type of goods, or
	 *         null if not found.
	 */
	 ...
	 * @param ability
	 *            An ability key.
	 * @return A <code>WorkLocation</code> with the required <code>Ability</code>,
	 ...
	 * @param modifier
	 *            A modifier key.
	 * @return A <code>WorkLocation</code> with the required <code>Modifier</code>,
	 *         or null if not found.
	 */
	 ...
	 * @return A list of <code>WorkLocation</code>s which consume the given type of
	 *         goods.
	 */
	 ...
	 * @param goodsType
	 *            The <code>GoodsType</code> to produce.
	 * @return A list of <code>WorkLocation</code>s which produce the given type of
	 *         goods.
	 */
	...
	/**
	 * Find a work location for producing a given type of goods. Beware that this
	 * may not be the optimal location for the production, for which
	 * {@link #getWorkLocationFor} is better.
	 *
	 * @param goodsType
	 *            The <code>GoodsType</code> to produce.
	 * @return A <code>WorkLocation</code>s which produces the given type of goods,
	 *         or null if not found.
	 */
	...
	/**
	 * Get a weighted list of natural disasters than can strike this colony. This
	 * list comprises all natural disasters that can strike the colony's tiles.
	 *
	...
	/**
	 * Is a building type able to be automatically built at no cost. True when the
	 * player has a modifier that collapses the cost to zero.
	 *
	 * @param buildingType
	 ...
	 * @return The number of turns to build the buildable, negative if some goods
	 *         are not being built, UNDEFINED if none is.
	 */
	 ...
	 *         (turns-to-blockage + 1), and if the needed argument is supplied it is
	 *         set to the goods deficit).
	 */
	...
	/**
	 * Returns <code>true</code> if this Colony can breed the given type of Goods.
	 * Only animals (such as horses) are expected to be breedable.
	 *
	 * @param goodsType
	...
	/**
	 * Return the reason why the give <code>BuildableType</code> can not be built.
	 *
	 * @param buildableType
	 *            A <code>BuildableType</code> to build.
	 * @param assumeBuilt
	 *            An optional list of other buildable types which can be assumed to
	 *            be built, for the benefit of build queue checks.
	 * @return A <code>NoBuildReason</code> value decribing the failure, including
...
//New code ends here
	 ...
	 * prerequisite raw materials as well as the direct requirements (i.e. hammers,
	 * tools). If enough of a required goods is present in the colony, then that
	 * type is not returned. Take care to order types with raw materials first so
	 * that we can prioritize gathering what is required before manufacturing.
	 *
	 ...
	 *                If the owner of this <code>Colony</code> has an insufficient
	 *                amount of gold.
	...
	/**
	 * Adds to the liberty points by increasing the liberty goods present. Used only
	 * by DebugMenu.
	 *
	 * @param amount
	 ...
	 * That is the number of extra units that can be added without damaging the
	 * production bonus, or the number of units to remove to improve it.
	 *
	 * @return The number of units to add to the colony, or if negative the negation
	 *         of the number of units to remove.
	 */
	 ...
	 *
	 * @return A <code>StringTemplate</code> describing why a colony can not reduce
	 ...
	 * Unit.setLocation when a unit moves into or out of this colony, but *not* if
	 * it is moving within the colony.
	 */
	...
	/**
	 * Signal to the colony that a unit is moving in or out or changing its internal
	 * work location to one with a different teaching ability. This requires either
	 * checking for a new teacher or student, or clearing any existing education
	 * relationships.
	 *
	 * @param unit
	 *            The <code>Unit</code> that is changing its education state.
	 * @param enable
	 *            If true, check for new education opportunities, otherwise clear
	 *            existing ones.
	 */
	 ...
	 * something worth pillaging. To be called by CombatModels when the attacker has
	 * defeated an unarmed colony defender.
	 *
	 * @param attacker
	 ...
	 *
	 * @return True if at least one piece of gold can be plundered from this colony.
	 */
	...
	/**
	 * Returns <code>true</code> if the number of enemy combat units on all tiles
	 * that belong to the colony exceeds the number of friendly combat units. At the
	 * moment, only the colony owner's own units are considered friendly, but that
	 * could be extended to include the units of allied players.
	 *
	...
	public int evaluateFor(Player player) {
		if (player.isAI() && player.getNumberOfSettlements() < 5) {// FIXME: magic#
	 ...
	 * unit type with a skill level not exceeding the level of the schoolhouse. @see
	 * Building#canAdd
	 *
	 * @param unit
	...
	/**
	 * Returns true if this colony has a schoolhouse and the unit type is a skilled
	 * unit type with a skill level not exceeding the level of the schoolhouse. The
	 * number of units already in the schoolhouse and the availability of pupils are
	 * not taken into account. @see Building#canAdd
	 *
	 * @param unitType
	 ...
	 * Consumers include all object that consume goods, e.g. Units, Buildings and
	 * BuildQueues.
	 *
	 ...
	 * production levels.
	 *
	 * @return The number of turns before starvation occurs, or negative if it will
	 *         not.
	 */
	...
	/**
	 * Get the current production <code>Modifier</code>, which is generated from the
	 ...
	 * @return True if something is being produced at the <code>WorkLocation</code>.
	 */
	...
	/**
	 * Gets a copy of the current production map. Useful in the server at the point
	 * net production is applied to a colony.
	 *
	 ...
	 *
	 * In general, all goods are useful. However post-independence there is no need
	 * for more liberty once Sol% reaches 100, nor immigration. Note the latter may
	 * change when we implement sailing to other European ports.
	 *
	 * @param goodsType
	 *            The <code>GoodsType</code> to check.
	 */
	 ...
	 *
	 * A simple colony.copy() can not work because all the colony tiles will be left
	 * referring to uncopied work tiles which the colony-copy does not own, which
	 * prevents them being used as valid work locations. We have to copy the colony
	 * tile (which includes the colony), and fix up all the colony tile work tiles
	 * to point to copies of the original tile, and fix the ownership of those
	 * tiles.
	 *
	 ...
	 * @return The corresponding <code>FreeColObject</code> in this colony, or null
	 *         if not found.
	 */
	...
	/**
	 * Check the integrity of the build queues. Catches build fails due to broken
	 * requirements.
	 *
	 * @param fix
